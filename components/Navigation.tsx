    ...
    <header className="flex justify-between items-center w-full px-4 md:px-6 h-16 sticky top-0 z-50 bg-neutral-900 md:bg-[#131313] border-b md:border-b-2 border-primary-container md:border-outline-variant">
      <div className="flex items-center gap-4 md:gap-8">
        ...
        <nav className="hidden md:flex items-center gap-6 font-headline uppercase tracking-tighter text-sm">
          <Link href="/" className={`${pathname === '/' ? 'text-primary border-b-2 border-primary pb-1 translate-y-[1px]' : 'text-[#cecece] hover:text-primary transition-colors'}`}>
            {t('nav_home')}
          </Link>
          <Link href="/calendar" className={`${pathname === '/calendar' ? 'text-primary border-b-2 border-primary pb-1 translate-y-[1px]' : 'text-[#cecece] hover:text-primary transition-colors'}`}>
            {t('nav_calendar')}
          </Link>
          <Link href="/search" className={`${pathname === '/search' ? 'text-primary border-b-2 border-primary pb-1 translate-y-[1px]' : 'text-[#cecece] hover:text-primary transition-colors'}`}>
            {t('nav_venues')}
          </Link>
          <Link href="/artists" className={`${pathname === '/artists' ? 'text-primary border-b-2 border-primary pb-1 translate-y-[1px]' : 'text-[#cecece] hover:text-primary transition-colors'}`}>
            {t('nav_artists')}
          </Link>
          <Link href="/tickets" className={`${pathname === '/tickets' ? 'text-primary border-b-2 border-primary pb-1 translate-y-[1px]' : 'text-[#cecece] hover:text-primary transition-colors'}`}>
            {t('nav_tickets')}
          </Link>
          <button
            onClick={toggle}
